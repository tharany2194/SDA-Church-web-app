...
const ALLOWED_MIME_TYPES = [
  ...
  'audio/mpeg', 'audio/mp3', 'audio/wav', 'audio/aac', 'audio/m4a', 'audio/ogg', 'audio/x-m4a', 'audio/mp4', 'audio/webm',
];
...
export async function parseBody(request, fileField = 'file') {
  ...
  if (contentType.includes('multipart/form-data')) {
    ...
    for (const [key, value] of formData.entries()) {
      ...
      } else if ((key === fileField || (fileField === 'file' && (key === 'image' || key === 'audio'))) && value.size > 0) {
        if (!ALLOWED_MIME_TYPES.includes(value.type)) {
          return {
            error: fail('File type not supported. Allowed: JPEG, PNG, WebP, GIF, MP4, WebM, MOV, Audio (MP3, WAV, M4A, AAC, OGG)', 415),
          };
        }
