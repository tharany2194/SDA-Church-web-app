'use client';
  ...
  export default function ArticleDetailPage() {
    ...
    const displayContent = language === 'ta' ? (article.contentTa || article.content) : article.content;
      ...
      <div className="min-h-screen bg-gray-50 py-10">
        <div className="container-custom max-w-3xl">
          <Link href="/articles" className="inline-flex items-center gap-2 text-gray-500 hover:text-primary-600 text-sm mb-6">
            <ArrowLeft size={16} /> {t[language].backToArticles}
          </Link>

          <article className="card overflow-hidden">
            {coverUrl && (
              <div className="relative h-64 md:h-80">
                <Image src={coverUrl} alt={displayTitle} fill className="object-cover" />
              </div>
            )}
            <div className="p-6 md:p-8">
              <div className="flex flex-wrap gap-2 mb-4">
                <span className="text-xs capitalize px-2.5 py-1 rounded-full bg-primary-100 text-primary-700 font-medium">{displayCategory}</span>
                {article.isFeatured && <span className="text-xs px-2.5 py-1 rounded-full bg-gold/20 text-yellow-700 font-medium">{t[language].featured}</span>}
              </div>

              <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">{displayTitle}</h1>
              {language === 'en' && article.titleTa && <p className="text-xl text-gray-600 mb-4">{article.titleTa}</p>}
              {language === 'ta' && article.titleTa && article.title !== article.titleTa && <p className="text-xl text-gray-600 mb-4">{article.title}</p>}
              ...
              <div className="flex flex-wrap gap-4 text-sm text-gray-500 mb-2">
                ...
                <span className="flex items-center gap-1"><Eye size={14} />{article.views} {t[language].views}</span>
              </div>
              ...
              {displayExcerpt && (
                <blockquote className="border-l-4 border-primary-500 pl-4 mb-6 italic text-gray-600">{displayExcerpt}</blockquote>
              )}
              ...
              <div className="flex flex-wrap gap-3 pt-6 border-t border-gray-100 mt-8">
                ...
                >
                  <Download size={15} /> {t[language].downloadPdf}
                </button>
              </div>
            </div>
          </article>
